...
//
// the zip comes from SoundBase's plugins/pack-plugin.mjs, which vendors
// @soundbase/plugin-shell and @soundbase/plugin-contract (neither is on npm),
// so a SoundBase checkout with plugins/ and a completed `pnpm install` is
// needed: --sb <path>, SB_ROOT, or the first ~/CODE/SoundBase* that has one.
//
...
if (!existsSync(join(SB, 'plugins/template/node_modules/@soundbase/plugin-shell'))) {
  fail(
...
if (!dryRun) {
  ...
  if (capture('git', ['ls-remote', '--tags', 'origin', `refs/tags/${tag}`])) {
    fail(`tag ${tag} already exists on origin: git push origin --delete ${tag}`);
  }
  ...
  run('git', ['commit', '-m', tag, '--', 'soundbase-plugin.json', 'package.json']);
  run('git', ['tag', tag]);
}
...
run('node', [join(SB, 'plugins/pack-plugin.mjs'), 'template', '--out', packDir], {
  cwd: SB,
});
...
copyFileSync(join(ROOT, 'scripts/smoke.mjs'), join(packDir, 'scripts/smoke.mjs'));
...
for (const f of ['soundbase-plugin.json', 'LICENSE', 'main.js', 'package.json']) {
...
if (packed.id !== id || packed.version !== version) {
  fail(`zip manifest says ${packed.id} ${packed.version}, expected ${id} ${version}`);
}
